...
/**
 * Per-instance in-memory rate limit for Business Copilot (user + tenant).
 */
@Component
public class BusinessCopilotRateLimiter {

	private final AiProperties properties;
	private final ConcurrentHashMap<String, Window> windows = new ConcurrentHashMap<>();

	public BusinessCopilotRateLimiter(AiProperties properties) {
	...
	public boolean tryAcquire(UUID businessId, UUID userId) {
		boolean userOk = tryAcquire(
				"u:" + businessId + ":" + userId,
				properties.getBusinessCopilot().getPerUserPerMinute());
		boolean tenantOk = tryAcquire(
				"t:" + businessId,
				properties.getBusinessCopilot().getPerTenantPerMinute());
